...
HvcP2.prototype._openSerialPort = function(p) {
	let promise = new Promise((resolve, reject) => {
		...
		if(p['path']) {
			this._tryOpenSerialPort(p).then(() => {
				resolve();
			}).catch((error) => {
				reject(error);
			});
			...
			let pf = mOs.platform();
			if(pf === 'linux') {
				this._tryOpenSerialPortLinux(p).then(() => {
					resolve();
				}).catch((error) => {
					reject(error);
				});
			} else if(pf === 'win32' || pf === 'darwin') {
				this._tryOpenSerialPortWin32Darwin(p, pf).then(() => {
					resolve();
				}).catch((error) => {
					reject(error);
				});
			} else {
				reject(new Error('Unknown platform. Specify the parameter `path`.'));
			}
		}
	});
	return promise;
...
HvcP2.prototype._tryOpenSerialPort = function(p) {
	let promise = new Promise((resolve, reject) => {
		let port = new mSerialPort(p['path'], {
			baudRate: p['baudRate']
		});
		...
		port.once('open', () => {
			...
			this.request(0x00, null).then((res) => {
				if(res['model'] === this._MODEL_NAME) {
					...
					this._serial_port_path = p['path'];
					resolve();
				} else {
					port.close(() => {
						port = null;
						reject(new Error('Unknown model: ' + res['model']));
					});
				}
			}).catch((error) => {
				reject(error);
...
HvcP2.prototype._tryOpenSerialPortLinux = function(p) {
	let error = new Error('Failed to find the serial port which the device is connected. Specify the parameter `path`.');
	let promise = new Promise((resolve, reject) => {
		let dir_path = '/dev/serial/by-id';
		if(!mFs.existsSync(dir_path)) {
			reject(error);
			return;
		}
		let fd = mFs.openSync(dir_path, 'r');
		let stats = mFs.fstatSync(fd);
		if(!stats.isDirectory()) {
			reject(error);
			return;
		}
		let file_list = mFs.readdirSync(dir_path);
		let fname = '';
		for(let i=0; i<file_list.length; i++) {
			if(file_list[i].match(/^usb\-OMRON_Corporation_OMRON_HVC\-P2/)) {
				fname = file_list[i];
				break;
			}
		}
		...
		let fpath = mPath.join(dir_path, fname);
		let lstat = mFs.lstatSync(fpath);
		if(!lstat.isSymbolicLink()) {
			reject(error);
			return;
		}
		let path = mFs.readlinkSync(fpath);
		if(!path) {
			reject(error);
			return;
		}
		path = mPath.join(dir_path, path);
...
HvcP2.prototype._tryOpenSerialPortWin32Darwin = function(p, pf) {
	let promise = new Promise((resolve, reject) => {
		let i = 0;
		let tryConnect = (cb) => {
			i ++;
			if(i > 8) {
				cb(new Error('Failed to find the serial port which the device is connected. Specify the parameter `path`.'));
				return;
			}
			let path = '';
			if(pf === 'win32') {
				...
				path = '/dev/tty.usbmodem' + i;
			} else {
				cb(new Error('Unknown platform. Specify the parameter `path`.'));
				return;
			}
			let port = new mSerialPort(path, {
				baudRate: p['baudRate']
